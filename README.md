<div align="center">

# Hi , I'm Srishti Kumari

<img src="https://readme-typing-svg.demolab.com?font=Poppins&weight=600&size=28&duration=3500&pause=1000&color=58A6FF&center=true&vCenter=true&width=900&lines=Full+Stack+Java+Developer;MCA+(AI+%26+ML)+Student;Spring+Boot+%7C+React+Developer;Machine+Learning+Explorer;DSA+Enthusiast;Open+Source+Learner" />

<br>

<img src="https://komarev.com/ghpvc/?username=SrishtiSoni25&label=Profile+Views&color=58A6FF&style=for-the-badge"/>

<img src="https://img.shields.io/github/followers/SrishtiSoni25?style=for-the-badge&color=58A6FF"/>

<img src="https://img.shields.io/github/stars/SrishtiSoni25?style=for-the-badge&color=58A6FF"/>

</div>

---

#  About Me

 **MCA (Artificial Intelligence & Machine Learning) Student**

Passionate Full Stack Java Developer who enjoys building scalable applications, solving real-world problems, and continuously learning modern technologies.

 Currently focused on developing enterprise-level applications using Java, Spring Boot, React, and Machine Learning.

###  Currently Learning

- Data Structures & Algorithms
- Spring Security
- Microservices
- System Design
- Machine Learning
- Docker & Kubernetes

### Ask Me About

- Java
- Spring Boot
- React
- REST APIs
- MySQL
- Machine Learning
- Git & GitHub

### Career Goal

Aspiring Software Development Engineer passionate about building impactful software and contributing to open-source projects.

---

#  Connect With Me

<p align="center">

<a href="https://www.linkedin.com/in/srishti-kumari-6135a8224/">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:srishtisoni2502@gmail.com">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://github.com/SrishtiSoni25">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github"/>
</a>

<a href="https://leetcode.com/u/Srishti_165/">
<img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=white"/>
</a>

</p>

---

#  Open To

-  Software Engineer
-  Full Stack Java Developer
-  Backend Developer
-  Frontend Developer
-  Open Source Collaboration
-  Internship Opportunities

---

#  Tech Stack

## Languages

<p align="center">

<img src="https://skillicons.dev/icons?i=java,python,javascript,html,css,mysql,springboot,C"/>

</p>

---

## Frontend

<p align="center">

<img src="https://skillicons.dev/icons?i=react,angular,bootstrap"/>

</p>

---

## Backend

<p align="center">

<img src="https://skillicons.dev/icons?i=spring"/>

<img src="https://img.shields.io/badge/Hibernate-59666C?style=for-the-badge&logo=hibernate&logoColor=white"/>

<img src="https://img.shields.io/badge/JDBC-007396?style=for-the-badge"/>

</p>

---

## AI / ML

<p align="center">

<img src="https://skillicons.dev/icons?i=tensorflow"/>

<img src="https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white"/>

<img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas"/>

<img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy"/>

</p>

---

## Database

<p align="center">

<img src="https://skillicons.dev/icons?i=mysql,mongodb"/>

</p>

---

## Tools

<p align="center">

<img src="https://skillicons.dev/icons?i=git,github,linux,vscode,vite,postman"/>

<img src="https://img.shields.io/badge/Tomcat-F8DC75?style=for-the-badge&logo=apachetomcat&logoColor=black"/>

</p>

---

# Core Skills

- Data Structures & Algorithms
- Object-Oriented Programming
- DBMS
- Operating Systems
- Computer Networks
- Software Engineering
- REST APIs
- Spring Boot
- React
- Problem Solving

---

# Current Focus

```yaml
Learning:
  - Spring Security
  - System Design
  - Microservices
  - Machine Learning

Building:
  - Full Stack Java Applications
  - REST APIs
  - React Projects

Exploring:
  - Docker
  - Kubernetes
  - AI Tools
  - Open Source

Goal:
  - Software Development Engineer
```

---
# Experience

## Full Stack Web Development Intern
### Investor Home Solutions

**Tech Stack:** Angular • TypeScript • Bootstrap • Firebase

- Developed responsive real estate web applications using Angular.
- Implemented reusable components for property listings.
- Integrated Firebase for backend data management.
- Improved UI responsiveness and application performance.
- Collaborated with the development team to deliver production-ready features.

---

## Cyber Security Intern
### Cyber Secured India

**Duration:** 3 Months

- Learned cybersecurity fundamentals and networking concepts.
- Worked on vulnerability assessment and security best practices.
- Gained practical exposure to ethical hacking methodologies.

---

##  Web Development Intern
### Oasis Infobyte

- Developed responsive websites using HTML, CSS and JavaScript.
- Improved frontend performance and user experience.
- Worked on real-world web development projects.

#  Featured Projects

---

##  Uttaranchal Alumni

### Alumni Networking Platform

**Tech Stack**

React • Node.js • Express.js • MySQL • Sequelize

### Description

- Developed a full-stack alumni networking platform.
- Secure authentication for students and alumni.
- Alumni directory with advanced search.
- Event and announcement management.
- Responsive user interface using React.
- REST APIs using Express.js.
- MySQL database managed with Sequelize ORM.

---

##  Music Factory

### Music Listing Web Application

**Tech Stack**

Java • Spring Boot • Hibernate • JDBC • Thymeleaf • MySQL

### Description

- Built a music listing platform with CRUD operations.
- Users can add, update, delete and browse songs.
- Implemented MVC architecture using Spring Boot.
- Integrated MySQL database using Hibernate.
- Responsive frontend using Thymeleaf templates.

---

##  FarmerQuest

### AI for Bharat Hackathon Project

**Tech Stack**

React • Spring Boot • AI • Machine Learning

### Description

- Gamified learning platform for sustainable agriculture.
- Interactive farming challenges.
- AI-assisted recommendations for farmers.
- Educational quizzes and progress tracking.
- Built to encourage smart farming practices.

---

##  Movie Recommendation System

### Machine Learning Project

**Tech Stack**

Python • Pandas • Scikit-Learn • Streamlit

### Description

- Built an ML-powered movie recommendation engine.
- Content-based recommendation algorithm.
- Interactive Streamlit web interface.
- Integrated TMDB API for posters.
- Fast recommendations using optimized similarity calculations.

---

##  Plant Disease Detection

### Machine Learning Project

**Tech Stack**

Python • TensorFlow • Keras • OpenCV

### Description

- Deep learning model to identify plant diseases.
- Trained using the PlantVillage dataset.
- Predicts disease from uploaded leaf images.
- Helps farmers detect diseases early.

---

# Achievements

-  Solved 100+ DSA problems on LeetCode.
- Completed the 100 Days of LeetCode Challenge.
-  Developed multiple Full Stack Java projects.
-  Built Machine Learning applications using Python.
-  Actively learning Microservices and System Design.
-  Hackathon Enthusiast.
- Open Source Learner.

---

# Certifications

###  Microsoft Certified

- Microsoft Certified: **DP-800 – SQL AI Developer Associate**

###  Artificial Intelligence

- Agentic AI Certification

###  Development

- Full Stack Java Development

###  Machine Learning

- Machine Learning using Python

---

# Education

| Degree | Institute | Duration |
|----------|-----------|----------|
| MCA (Artificial Intelligence & Machine Learning) | Pursuing | 2025 – 2027 |
| Bachelor of Computer Applications (BCA) | Completed | 2021 – 2024 |

---
# Coding Profiles

<p align="center">

<a href="https://leetcode.com/u/Srishti_165/" target="_blank">
<img src="https://img.shields.io/badge/LeetCode-Profile-FFA116?style=for-the-badge&logo=leetcode&logoColor=white"/>
</a>

<a href="https://github.com/SrishtiSoni25" target="_blank">
<img src="https://img.shields.io/badge/GitHub-Profile-181717?style=for-the-badge&logo=github"/>
</a>

<a href="https://www.linkedin.com/in/srishti-kumari-6135a8224/" target="_blank">
<img src="https://img.shields.io/badge/LinkedIn-Profile-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

</p>

---

<p align="center">

<img src="https://streak-stats.demolab.com?user=SrishtiSoni25&theme=tokyonight&hide_border=true"/>

</p>

---

# 📈 GitHub Activity Graph

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=SrishtiSoni25&theme=tokyo-night&hide_border=true"/>

</p>

---

#  Contribution Snake

> **⚠️ This image will appear only after you set up the GitHub Actions workflow.**

<p align="center">

<img src="https://raw.githubusercontent.com/SrishtiSoni25/SrishtiSoni25/output/github-contribution-grid-snake-dark.svg" alt="Contribution Snake"/>

</p>

---

# 💡 Developer Mindset

```text
while(!success) {
    learn();
    build();
    debug();
    improve();
}
```

---

# 🌟 Fun Facts

- ☕ Coffee + Music = Productive Coding Sessions
- 💻 I enjoy turning ideas into real-world applications.
- 📚 Always exploring new technologies.
- 🤝 Love collaborating on open-source projects.
- 🚀 Passionate about solving real-world problems with code.

---

# 🎯 2026 Goals

-  Master Data Structures & Algorithms
-  Build scalable Full Stack Java applications
-  Learn Microservices & System Design
-  Contribute to Open Source Projects
-  Strengthen AI & Machine Learning skills
-  Crack Software Development Engineer (SDE) interviews

---

#  Let's Connect

<p align="center">

<a href="mailto:srishtisoni2502@gmail.com">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/srishti-kumari-6135a8224/">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/SrishtiSoni25">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github"/>
</a>

<a href="https://leetcode.com/u/Srishti_165/">
<img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=white"/>
</a>

</p>

---

#  Quote

<p align="center">

<img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight"/>

</p>

---

<div align="center">

## ⭐ Thanks for visiting my profile!

<img src="https://capsule-render.vercel.app/api?type=waving&height=140&section=footer&color=0:0D1117,25:1F2937,50:2563EB,75:3B82F6,100:60A5FA"/>

### 💙 If you like my work, consider giving a ⭐ to my repositories!

<img src="https://komarev.com/ghpvc/?username=SrishtiSoni25&label=Visitors&color=58A6FF&style=flat-square"/>

</div>
